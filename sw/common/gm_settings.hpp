...
public:
  AntGlobals &antg;
  GameSirenSettings(AntGlobals &antg) : antg(antg) {}
...
class GameSettings {
private:
  enum class STATE { TOP, SIREN };
  STATE state = STATE::TOP;

  enum class MENU { SIREN, BACK, COUNT };
  MENU menu = MENU::SIREN;

  AntGlobals &antg;
  GameSirenSettings siren_settings;

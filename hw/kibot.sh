...
function run_kibot() {
	...
	--workdir "/tmp/workdir" \
	setsoft/kicad_auto:ki8 \
	/bin/bash -c "groupadd -g$gid u; useradd -u$uid -g$gid -d/tmp u; su u -c 'cd airsoft_bomb_v2 && kibot -c .kibot.yaml $*'"
}
...
# regenerate qr code
mkdir -p gen
#sch_version="$(grep '\(rev "[^"]*\)"' airsoft_bomb_v2/airsoft_bomb_v2.kicad_sch | cut -d'"' -f2)"
echo "https://github.com/makerspacelt/airsoft-bomb-prop${sch_version}" \
